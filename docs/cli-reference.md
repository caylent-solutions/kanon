...
These flags are accepted by `kanon` itself and apply to every subcommand. They
are registered once via `kanon_cli.core.cli_args` so every parser exposes them.

| Flag | Env var | Default | Description |
|------|---------|---------|-------------|
| `--version` | -- | -- | Print the kanon version and exit. |
...
| `--allow-abs-root <path>` | `KANON_ALLOWED_ABS_ROOTS` | (none) | Permit an absolute `<linkfile>`/`<copyfile>` dest to resolve under `<path>`. Repeatable. The consumer project root and the resolved `CLAUDE_MARKETPLACES_DIR` are always permitted and cannot be removed, so this only ever widens the boundary. Precedence: flag > env var > the two built-in roots. |
...
- **dest**: `catalog_source`
- **metavar**: `<git-url>@<ref>`
- **env var**: `KANON_CATALOG_SOURCES` (constant `CATALOG_SOURCES_ENV_VAR` in
  `kanon_cli.constants`)
...
| `KANON_ALLOWED_ABS_ROOTS` | Extra roots an absolute `<linkfile>`/`<copyfile>` dest may resolve under, separated by the platform path separator. Entries must be absolute paths. Overridden by `--allow-abs-root`. See [docs/security-model.md](security-model.md). | (none) |
| `KANON_SYNC_JOBS` | Upper bound on `repo sync` worker processes. A cap: it lowers fan-out, never raises it. Must be a positive integer; validated at CLI entry. | (unset) |
...
| `KANON_WHY_JSON_INDENT` | JSON indentation (number of spaces) used by `kanon why --format json`. | `2` |
...
1. **Catalog required** -- `--catalog-source` or a single `KANON_CATALOG_SOURCES`
   entry must be set. When neither is present the command exits non-zero with
   `ERROR: kanon outdated requires a catalog source.` followed by the remediation
...
every node already has its resolved SHA). When no lockfile is present, `kanon why`
...
2. Resolve the full dependency tree:
   ...
   - Otherwise, live-resolve by cloning the catalog source and walking each manifest
     ...
     (`--catalog-source` or a single `KANON_CATALOG_SOURCES` entry) is required on
     this path; when neither is configured the command exits non-zero with
     `ERROR: kanon why requires a catalog source.` and the remediation block.

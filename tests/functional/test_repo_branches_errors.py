"""Functional tests for 'kanon repo branches' error paths and --help.

Verifies that:
- 'kanon repo branches --help' exits 0 with usage text (AC-TEST-001).
...
def _assert_deterministic(
    ...
    """Run 'kanon repo branches [extra_args]' twice and assert output-channel equality.
    ...
    Args:
        ...
        extra_args: CLI arguments appended after 'kanon repo branches'.
    ...
    result_a = _run_kanon(_CLI_TOKEN_REPO, _CLI_FLAG_REPO_DIR, repo_dir, _CLI_TOKEN_BRANCHES, *extra_args)
    result_b = _run_kanon(_CLI_TOKEN_REPO, _CLI_FLAG_REPO_DIR, repo_dir, _CLI_TOKEN_BRANCHES, *extra_args)
...
class TestRepoBranchesHelp:
    ...
    def test_help_flag_exits_zero_with_usage_on_stdout_and_empty_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --help' exits 0, emits usage text on stdout, and has empty stderr.
    ...
    def test_help_flag_stdout_mentions_jobs_option(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --help' stdout must document the --jobs option.
    ...
    def test_help_flag_output_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --help' produces the same output on repeated calls.
...
class TestRepoBranchesUnknownFlag:
    ...
    def test_unknown_flag_exits_2_with_flag_in_stderr_not_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --unknown-flag-xyzzy' exits 2, names the flag in stderr, not stdout.
    ...
    def test_unknown_flag_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --unknown-flag-xyzzy' produces the same error on repeated calls.
...
class TestRepoBranchesMissingRequiredPositional:
    ...
    def test_jobs_without_value_exits_2_with_option_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --jobs' without a value exits 2 and names the option in stderr.
    ...
    def test_verbose_with_inline_value_exits_2(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --verbose=unexpected' exits 2 with option name in stderr.
    ...
    def test_missing_required_option_value_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --jobs' (no value) produces the same error on repeated calls.
...
class TestRepoBranchesPreconditionFailure:
    ...
    def test_missing_repo_dir_exits_1(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches' without .repo exits 1.
    ...
    def test_missing_repo_dir_error_phrase_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches' without .repo emits 'error parsing manifest' on stderr.
    ...
    def test_missing_repo_dir_manifest_filename_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches' without .repo names 'manifest.xml' in stderr.
    ...
    def test_missing_repo_dir_error_not_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches' without .repo must not emit the error to stdout.
    ...
    def test_missing_repo_dir_stderr_is_non_empty(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches' without .repo must produce non-empty stderr.
    ...
    def test_missing_repo_dir_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches' without .repo produces the same error on repeated calls.
...
class TestRepoBranchesErrorChannelDiscipline:
    """AC-FUNC-001 / AC-CHANNEL-001: Channel discipline for 'kanon repo branches' error paths.
    ...
    def test_unknown_flag_error_is_stderr_only(self, tmp_path: pathlib.Path) -> None:
        ...
        Supplies an unrecognised flag to 'kanon repo branches' and verifies that
    ...
    def test_missing_option_value_error_is_stderr_only(self, tmp_path: pathlib.Path) -> None:
        ...
        Supplies '--jobs' without a value to 'kanon repo branches' and verifies
    ...
    def test_precondition_failure_error_is_stderr_only(self, tmp_path: pathlib.Path) -> None:
        ...
        Invokes 'kanon repo branches' against a nonexistent .repo directory and
    ...
    def test_help_flag_output_is_stdout_only(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo branches --help' output must appear on stdout only, not stderr.

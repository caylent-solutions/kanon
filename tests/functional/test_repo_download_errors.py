"""Functional tests for 'kanon repo download' error paths and --help.

Verifies that:
- 'kanon repo download --help' exits 0 with usage text (AC-TEST-001).
...
- 'kanon repo download --help'              -> exit 0, output on stdout only.
- 'kanon repo download --unknown-flag X'    -> exit 2, error on stderr.
- 'kanon repo download --branch'            -> exit 2, error on stderr.
- 'kanon repo download' (no args, no .repo) -> exit 1, UsageError on stderr.
- 'kanon repo download X/1' (no .repo)      -> exit 1, manifest error on stderr.
...
# No inline literals in test logic.
# ---------------------------------------------------------------------------

# Nonexistent repo-dir path component used in argument-parser and
...
# Value-requiring option used in AC-TEST-003: '--branch' requires exactly one
# string argument. Supplying it without a value triggers exit 2 with
# "'--branch option requires 1 argument'" on stderr.
_OPTION_REQUIRING_VALUE = "--branch"
_OPTION_REQUIRING_VALUE_SHORT = "-b"

# A valid change/patchset argument used in argument-parser-level tests.
# These tests exercise flag or option errors and do not require .repo to exist.
_DUMMY_CHANGE_ARG = "999/1"

# Error message substring expected in stderr when the '--branch' option is
# supplied without its required value (argument-parser missing-argument error).
...
# Phrase expected in --help output describing the positional argument format.
...
# Phrase expected in stderr when no positional arguments are supplied.
# The embedded tool raises UsageError (exit 1), not an argparse error (exit 2).
...
def _assert_deterministic(
    ...
    """Run 'kanon repo download [extra_args]' twice and assert output channel equality.
    ...
    Args:
        ...
        extra_args: CLI arguments appended after 'kanon repo download'.
    ...
    result_a = _run_kanon(_CLI_TOKEN_REPO, _CLI_FLAG_REPO_DIR, repo_dir, _CLI_TOKEN_DOWNLOAD, *extra_args)
    result_b = _run_kanon(_CLI_TOKEN_REPO, _CLI_FLAG_REPO_DIR, repo_dir, _CLI_TOKEN_DOWNLOAD, *extra_args)
    ...
    assert output_a == output_b, (
        f"'kanon repo download {extra_args}' produced different {channel_name} on repeated calls.\n"
        f"  first:  {output_a!r}\n"
        f"  second: {output_b!r}"
    )
...
# ---------------------------------------------------------------------------
# AC-TEST-001: 'kanon repo download --help' exits 0 with usage text
# ---------------------------------------------------------------------------
...
class TestRepoDownloadHelp:
    ...
    def test_help_flag_exits_zero_with_stdout_and_empty_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download --help' exits 0, emits non-empty stdout, and has empty stderr.
        ...
        assert result.returncode == _EXIT_SUCCESS, (
            f"'kanon repo download --help' exited {result.returncode}, "
            f"expected {_EXIT_SUCCESS}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
        assert len(result.stdout) > 0, (
            f"'kanon repo download --help' produced empty stdout; "
            f"usage text must appear on stdout.\n"
            f"  stderr: {result.stderr!r}"
        )
        assert _HELP_USAGE_PHRASE in result.stdout, (
            f"Expected {_HELP_USAGE_PHRASE!r} in stdout of 'kanon repo download --help'.\n  stdout: {result.stdout!r}"
        )
        assert len(result.stderr) == 0, (
            f"'kanon repo download --help' produced unexpected stderr output.\n  stderr: {result.stderr!r}"
        )

    def test_help_flag_stdout_mentions_change_positional(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download --help' stdout must document the change positional argument.
        ...
        assert result.returncode == _EXIT_SUCCESS, (
            f"Prerequisite: 'kanon repo download --help' failed.\n  stderr: {result.stderr!r}"
        )
        assert _HELP_POSITIONAL_PHRASE in result.stdout.lower(), (
            f"Expected {_HELP_POSITIONAL_PHRASE!r} in stdout of 'kanon repo download --help'.\n"
            f"  stdout: {result.stdout!r}"
        )

    def test_help_flag_output_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download --help' produces the same output on repeated calls.
...
class TestRepoDownloadUnknownFlag:
    ...
    def test_unknown_flag_exits_2_with_flag_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download --unknown-flag-xyzzy' exits 2, names the flag in stderr.
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo download {_DUMMY_CHANGE_ARG} {_UNKNOWN_FLAG_PRIMARY}' exited "
    ...
    def test_various_unknown_flags_exit_2_with_flag_in_stderr(
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo download {_DUMMY_CHANGE_ARG} {bad_flag}' exited "
            f"{result.returncode}, expected {_EXIT_ARGPARSE_ERROR}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
    ...
    def test_unknown_flag_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download 999/1 --unknown-flag-xyzzy' produces the same error on repeated calls.
...
class TestRepoDownloadMissingOptionValue:
    ...
    def test_no_positional_args_exits_1_with_usage_error(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download' with no args exits 1 (UsageError), NOT 2 (argparse).
        ...
        """
        repo_dir = str(tmp_path / _NONEXISTENT_REPO_DIR_NAME)
        result = _run_kanon(_CLI_TOKEN_REPO, _CLI_FLAG_REPO_DIR, repo_dir, _CLI_TOKEN_DOWNLOAD)
        assert result.returncode == _EXIT_PRECONDITION_ERROR, (
            f"'kanon repo download' (no args) exited {result.returncode}, "
    ...
    def test_branch_without_value_exits_2_with_option_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download --branch' without a value exits 2 and names the option in stderr.
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo download {_OPTION_REQUIRING_VALUE}' (no value) exited "
            f"{result.returncode}, expected {_EXIT_ARGPARSE_ERROR}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
    ...
    def test_various_options_without_value_exit_2(self, tmp_path: pathlib.Path, option_flag: str) -> None:
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo download {option_flag}' (no value) exited "
            f"{result.returncode}, expected {_EXIT_ARGPARSE_ERROR}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
    ...
    def test_missing_required_option_value_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download --branch' (no value) produces the same error on repeated calls.
...
class TestRepoDownloadPreconditionFailure:
    ...
    def test_missing_repo_dir_exits_1(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download <change>' without .repo exits 1.
        ...
        assert result.returncode == _EXIT_PRECONDITION_ERROR, (
            f"'kanon repo download {_DUMMY_CHANGE_ARG}' (no .repo) exited "
    ...
    def test_missing_repo_dir_error_phrase_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download <change>' without .repo emits 'error parsing manifest' on stderr.
    ...
    def test_missing_repo_dir_manifest_filename_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download <change>' without .repo names 'manifest.xml' in stderr.
    ...
    def test_missing_repo_dir_error_not_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download <change>' without .repo must not emit the error to stdout.
    ...
    def test_missing_repo_dir_stderr_is_non_empty(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download <change>' without .repo must produce non-empty stderr.
        ...
        assert result.returncode == _EXIT_PRECONDITION_ERROR
        assert len(result.stderr) > 0, (
            f"'kanon repo download {_DUMMY_CHANGE_ARG}' (no .repo) produced empty stderr; "
            f"error message must appear on stderr."
        )

    def test_missing_repo_dir_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download <change>' without .repo produces the same error on repeated calls.
...
class TestRepoDownloadErrorChannelDiscipline:
    """AC-FUNC-001 / AC-CHANNEL-001: Channel discipline for 'kanon repo download' error paths.
    ...
    def test_unknown_flag_error_is_stderr_only(self, tmp_path: pathlib.Path) -> None:
        ...
        Supplies an unrecognised flag to 'kanon repo download' and verifies
    ...
    def test_missing_option_value_error_is_stderr_only(self, tmp_path: pathlib.Path) -> None:
        ...
        Supplies '--branch' without a value to 'kanon repo download' and
    ...
    def test_precondition_failure_error_is_stderr_only(self, tmp_path: pathlib.Path) -> None:
        ...
        Invokes 'kanon repo download' against a nonexistent .repo directory
    ...
    def test_help_flag_output_is_stdout_only(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo download --help' output must appear on stdout only, not stderr.
        ...
        assert len(result.stdout) > 0, (
            "'kanon repo download --help' produced empty stdout; usage text must appear on stdout."
        )
        assert len(result.stderr) == 0, (
            f"'kanon repo download --help' produced unexpected stderr output.\n  stderr: {result.stderr!r}"
        )

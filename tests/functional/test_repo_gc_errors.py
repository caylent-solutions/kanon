"""Functional tests for 'kanon repo gc' error paths and --help.

Verifies that:
- 'kanon repo gc --help' exits 0 with usage text (AC-TEST-001).
...
# 'repo gc' defines --dry-run as a boolean store_true flag; the optparse
# parser rejects '--dry-run=unexpected' because boolean flags cannot accept
# an inline value, producing exit 2 with '--dry-run option does not take a value'.
...
# ---------------------------------------------------------------------------
# AC-TEST-001: 'kanon repo gc --help' exits 0 with usage text
# ---------------------------------------------------------------------------
...
class TestRepoGcHelp:
    ...
    def test_help_flag_exits_zero(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --help' must exit with code 0.
        ...
        assert result.returncode == _EXIT_SUCCESS, (
            f"'kanon repo gc --help' exited {result.returncode}, "
    ...
    def test_help_flag_produces_output_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --help' must produce non-empty output on stdout.
        ...
        assert result.returncode == _EXIT_SUCCESS, (
            f"Prerequisite: 'kanon repo gc --help' failed with exit {result.returncode}.\n  stderr: {result.stderr!r}"
        )
        assert len(result.stdout) > 0, (
            f"'kanon repo gc --help' produced empty stdout; "
            f"usage text must appear on stdout.\n"
            f"  stderr: {result.stderr!r}"
        )

    def test_help_flag_stdout_contains_usage_phrase(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --help' stdout must contain the phrase 'repo gc'.
        ...
        assert result.returncode == _EXIT_SUCCESS, (
            f"Prerequisite: 'kanon repo gc --help' failed.\n  stderr: {result.stderr!r}"
        )
        assert _HELP_USAGE_PHRASE in result.stdout, (
            f"Expected {_HELP_USAGE_PHRASE!r} in stdout of 'kanon repo gc --help'.\n  stdout: {result.stdout!r}"
        )

    def test_help_flag_stderr_is_empty(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --help' must not produce any error output on stderr.
        ...
        assert len(result.stderr) == 0, (
            f"'kanon repo gc --help' produced unexpected stderr output.\n  stderr: {result.stderr!r}"
        )

    def test_help_flag_stdout_mentions_dry_run_option(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --help' stdout must document the --dry-run option.
        ...
        assert _BOOL_FLAG_BASE_NAME in result.stdout, (
            f"Expected {_BOOL_FLAG_BASE_NAME!r} documented in stdout of 'kanon repo gc --help'.\n  stdout: {result.stdout!r}"
        )

    def test_help_flag_output_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --help' produces the same output on repeated calls.
        ...
        assert result_a.stdout == result_b.stdout, (
            f"'kanon repo gc --help' produced different stdout on repeated calls.\n"
...
class TestRepoGcUnknownFlag:
    ...
    def test_unknown_flag_exits_2(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --unknown-flag-xyzzy' must exit with code 2.
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo gc {_UNKNOWN_FLAG_PRIMARY}' exited {result.returncode}, "
    ...
    def test_unknown_flag_names_the_flag_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --unknown-flag-xyzzy' stderr must contain the flag name.
    ...
    def test_unknown_flag_error_phrase_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --unknown-flag-xyzzy' stderr must contain 'no such option'.
    ...
    def test_unknown_flag_error_not_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --unknown-flag-xyzzy' must not leak the error to stdout.
    ...
    def test_various_unknown_flags_exit_2(self, tmp_path: pathlib.Path, bad_flag: str) -> None:
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo gc {bad_flag}' exited {result.returncode}, "
            f"expected {_EXIT_ARGPARSE_ERROR}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
    ...
    def test_unknown_flag_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --unknown-flag-xyzzy' produces the same error on repeated calls.
        ...
        assert result_a.stderr == result_b.stderr, (
            f"'kanon repo gc {_UNKNOWN_FLAG_PRIMARY}' produced different stderr on "
            f"repeated calls.\n"
...
class TestRepoGcBoolFlagWithValue:
    ...
    def test_bool_flag_with_value_exits_2(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --dry-run=unexpected' must exit with code 2.
        ...
        '--dry-run option does not take a value' and exiting 2. The kanon
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo gc {_BOOL_FLAG_WITH_VALUE}' exited {result.returncode}, "
            f"expected {_EXIT_ARGPARSE_ERROR}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
    ...
    def test_bool_flag_with_value_names_option_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --dry-run=unexpected' error must name the flag base name in stderr.
    ...
    def test_bool_flag_with_value_does_not_take_value_phrase_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --dry-run=unexpected' stderr must contain 'does not take a value'.
    ...
    def test_bool_flag_with_value_error_not_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --dry-run=unexpected' must not leak the error to stdout.
    ...
    def test_various_bool_flags_with_values_exit_2(self, tmp_path: pathlib.Path, bad_token: str) -> None:
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo gc {bad_token}' exited {result.returncode}, "
            f"expected {_EXIT_ARGPARSE_ERROR}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
    ...
    def test_bool_flag_with_value_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --dry-run=unexpected' produces the same error on repeated calls.
        ...
        assert result_b.returncode == _EXIT_ARGPARSE_ERROR
        assert result_a.stderr == result_b.stderr, (
            f"'kanon repo gc {_BOOL_FLAG_WITH_VALUE}' produced different stderr on "
            f"repeated calls.\n"
            f"  first:  {result_a.stderr!r}\n"
            f"  second: {result_b.stderr!r}"
        )
...
class TestRepoGcPreconditionFailure:
    ...
    def test_missing_repo_dir_exits_1(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc' with a nonexistent .repo directory must exit with code 1.
        ...
        assert result.returncode == _EXIT_PRECONDITION_ERROR, (
            f"'kanon repo gc' (no .repo dir) exited {result.returncode}, "
    ...
    def test_missing_repo_dir_error_phrase_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc' without .repo must emit 'error parsing manifest' on stderr.
    ...
    def test_missing_repo_dir_error_not_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc' without .repo must not emit the error to stdout.
        ...
        assert result.returncode == _EXIT_PRECONDITION_ERROR
        assert len(result.stdout) == 0, (
            f"'kanon repo gc' (no .repo dir) produced unexpected stdout output.\n  stdout: {result.stdout!r}"
        )

    def test_missing_repo_dir_stderr_is_non_empty(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc' without .repo must produce non-empty stderr output.
        ...
        assert result.returncode == _EXIT_PRECONDITION_ERROR
        assert len(result.stderr) > 0, (
            f"'kanon repo gc' (no .repo dir) produced empty stderr; error must appear on stderr.\n"
            f"  stdout: {result.stdout!r}"
        )

    def test_missing_repo_dir_names_manifest_file_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc' without .repo must name the manifest file in stderr.
    ...
    def test_missing_repo_dir_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc' without .repo produces the same error on repeated calls.
        ...
        assert result_a.stderr == result_b.stderr, (
            f"'kanon repo gc' (no .repo dir) produced different stderr on repeated calls.\n"
            f"  first:  {result_a.stderr!r}\n"
            f"  second: {result_b.stderr!r}"
        )
...
class TestRepoGcErrorChannelDiscipline:
    ...
    by 'kanon repo gc' appear on stderr only, and that stdout remains clean
    ...
    def test_help_output_on_stdout_not_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo gc --help' must route help text to stdout, not stderr.
        ...
        assert result.returncode == _EXIT_SUCCESS
        assert len(result.stdout) > 0, (
            f"'kanon repo gc --help' produced no stdout; help must appear on stdout.\n  stderr: {result.stderr!r}"
        )
        assert len(result.stderr) == 0, (
            f"'kanon repo gc --help' produced unexpected stderr.\n  stderr: {result.stderr!r}"
        )

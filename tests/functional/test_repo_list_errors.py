"""Functional tests for 'kanon repo list' error paths and --help.

Verifies that:
- 'kanon repo list --help' exits 0 with usage text (AC-TEST-001).
...
class TestRepoListHelp:
    ...
    def test_help_flag_exits_zero(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --help' must exit with code 0.
        ...
        assert result.returncode == _EXIT_SUCCESS, (
            f"'kanon repo list --help' exited {result.returncode}, "
    ...
    def test_help_flag_produces_output_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --help' must produce non-empty output on stdout.
        ...
        assert result.returncode == _EXIT_SUCCESS, (
            f"Prerequisite: 'kanon repo list --help' failed with exit {result.returncode}.\n  stderr: {result.stderr!r}"
        )
        assert len(result.stdout) > 0, (
            f"'kanon repo list --help' produced empty stdout; "
            f"usage text must appear on stdout.\n"
            f"  stderr: {result.stderr!r}"
        )

    def test_help_flag_stdout_contains_usage_phrase(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --help' stdout must contain the phrase 'repo list'.
        ...
        assert result.returncode == _EXIT_SUCCESS, (
            f"Prerequisite: 'kanon repo list --help' failed.\n  stderr: {result.stderr!r}"
        )
        assert _HELP_USAGE_PHRASE in result.stdout, (
            f"Expected {_HELP_USAGE_PHRASE!r} in stdout of 'kanon repo list --help'.\n  stdout: {result.stdout!r}"
        )

    def test_help_flag_stderr_is_empty(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --help' must not produce any error output on stderr.
        ...
        assert len(result.stderr) == 0, (
            f"'kanon repo list --help' produced unexpected stderr output.\n  stderr: {result.stderr!r}"
        )

    def test_help_flag_stdout_mentions_documented_flag(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --help' stdout must document the --groups option.
        ...
        assert _HELP_DOCUMENTED_FLAG in result.stdout, (
            ...
            f"'kanon repo list --help'.\n  stdout: {result.stdout!r}"
        )

    def test_help_flag_output_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --help' produces the same output on repeated calls.
        ...
        assert result_a.stdout == result_b.stdout, (
            f"'kanon repo list --help' produced different stdout on repeated calls.\n"
...
class TestRepoListUnknownFlag:
    ...
    def test_unknown_flag_exits_2(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --unknown-flag-xyzzy' must exit with code 2.
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo list {_UNKNOWN_FLAG_PRIMARY}' exited {result.returncode}, "
    ...
    def test_unknown_flag_names_the_flag_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --unknown-flag-xyzzy' stderr must contain the flag name.
    ...
    def test_unknown_flag_error_phrase_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --unknown-flag-xyzzy' stderr must contain 'no such option'.
    ...
    def test_unknown_flag_error_not_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --unknown-flag-xyzzy' must not leak the error to stdout.
    ...
    def test_various_unknown_flags_exit_2(self, tmp_path: pathlib.Path, bad_flag: str, test_id: str) -> None:
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo list {bad_flag}' ({test_id}) exited {result.returncode}, "
            f"expected {_EXIT_ARGPARSE_ERROR}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
    ...
    def test_unknown_flag_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --unknown-flag-xyzzy' produces the same error on repeated calls.
        ...
        assert result_a.stderr == result_b.stderr, (
            f"'kanon repo list {_UNKNOWN_FLAG_PRIMARY}' produced different stderr on "
            f"repeated calls.\n"
...
class TestRepoListMissingRequiredArg:
    ...
    def test_option_without_required_value_exits_2(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --groups' without a value must exit with code 2.

        The embedded optparse parser rejects '--groups' when no value follows,
        emitting '--groups option requires 1 argument' and exiting 2. The kanon
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo list {_OPTION_REQUIRING_VALUE_PRIMARY}' (no value) exited "
    ...
    def test_option_without_required_value_requires_phrase_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --groups' without a value produces 'requires' in stderr.
    ...
    def test_option_without_required_value_names_option_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --groups' without a value names the option in stderr.
    ...
    def test_option_without_required_value_error_not_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --groups' without a value must not leak error to stdout.
    ...
    def test_various_options_without_value_exit_2(self, tmp_path: pathlib.Path, option: str, test_id: str) -> None:
        ...
        assert result.returncode == _EXIT_ARGPARSE_ERROR, (
            f"'kanon repo list {option}' ({test_id}) with no value exited "
            f"{result.returncode}, expected {_EXIT_ARGPARSE_ERROR}.\n"
            f"  stdout: {result.stdout!r}\n"
            f"  stderr: {result.stderr!r}"
        )
    ...
    def test_option_without_required_value_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --groups' (no value) produces the same error on repeated calls.
        ...
        assert result_b.returncode == _EXIT_ARGPARSE_ERROR
        assert result_a.stderr == result_b.stderr, (
            f"'kanon repo list {_OPTION_REQUIRING_VALUE_PRIMARY}' produced different "
...
class TestRepoListPreconditionFailure:
    ...
    propagated correctly by the kanon layer.
    """

    def test_missing_repo_dir_exits_1(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list' with a nonexistent .repo directory must exit with code 1.
        ...
        assert result.returncode == _EXIT_PRECONDITION_ERROR, (
            f"'kanon repo list' (no .repo dir) exited {result.returncode}, "
    ...
    def test_missing_repo_dir_error_phrase_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list' without .repo must emit 'error parsing manifest' on stderr.
    ...
    def test_missing_repo_dir_error_not_on_stdout(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list' without .repo must not emit the error to stdout.
        ...
        assert result.returncode == _EXIT_PRECONDITION_ERROR
        assert len(result.stdout) == 0, (
            f"'kanon repo list' (no .repo dir) produced unexpected stdout output.\n  stdout: {result.stdout!r}"
        )

    def test_missing_repo_dir_stderr_is_non_empty(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list' without .repo must produce non-empty stderr output.
        ...
        assert result.returncode == _EXIT_PRECONDITION_ERROR
        assert len(result.stderr) > 0, (
            f"'kanon repo list' (no .repo dir) produced empty stderr; "
            f"error must appear on stderr.\n"
            f"  stdout: {result.stdout!r}"
        )

    def test_missing_repo_dir_names_manifest_file_in_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list' without .repo must name the manifest file in stderr.
    ...
    def test_missing_repo_dir_error_is_deterministic(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list' without .repo produces the same error on repeated calls.
        ...
        assert result_a.stderr == result_b.stderr, (
            f"'kanon repo list' (no .repo dir) produced different stderr on "
            f"repeated calls.\n"
            f"  first:  {result_a.stderr!r}\n"
            f"  second: {result_b.stderr!r}"
        )
...
class TestRepoListErrorChannelDiscipline:
    ...
    by 'kanon repo list' appear on stderr only, and that stdout remains clean
    ...
    def test_help_output_on_stdout_not_stderr(self, tmp_path: pathlib.Path) -> None:
        """'kanon repo list --help' must route help text to stdout, not stderr.
        ...
        assert result.returncode == _EXIT_SUCCESS
        assert len(result.stdout) > 0, (
            f"'kanon repo list --help' produced no stdout; help must appear on stdout.\n  stderr: {result.stderr!r}"
        )
        assert len(result.stderr) == 0, (
            f"'kanon repo list --help' produced unexpected stderr.\n  stderr: {result.stderr!r}"
        )

...
Covers the MS-01 test class and CD-01/CD-02 test classes from docs/integration-testing.md:
  - MS-01: Two sources with distinct packages aggregate correctly
  - CD-01/CD-02: Two sources with conflicting paths surface a collision error
...
# URL template used in .kanon fixture content for mocked sources.
# The actual URL is never contacted -- all repo calls are patched out.
_FIXTURE_SOURCE_URL_TEMPLATE = "https://example.com/{source_name}.git"

# Number of install runs used to verify idempotency across multiple executions.
...
def _two_source_kanonenv_content(source_alpha: str, source_bravo: str) -> str:
    ...
    """
    url_alpha = _FIXTURE_SOURCE_URL_TEMPLATE.format(source_name=source_alpha)
    url_bravo = _FIXTURE_SOURCE_URL_TEMPLATE.format(source_name=source_bravo)
    return (
...
# ---------------------------------------------------------------------------
# AC-TEST-002: CD-01/CD-02 class -- conflicting paths surface collision error
# ---------------------------------------------------------------------------


@pytest.mark.integration
class TestCD01CD02ConflictingPathsCollisionError:
    """CD-01/CD-02: Two sources with the same package path surface a collision error.
...
class TestAggregationStabilityAcrossReRuns:
    ...
    def test_package_count_unchanged_after_multiple_installs(self, tmp_path: Path) -> None:
        ...
        for _run in range(_NUM_STABILITY_RUNS):
            _install_with_packages(kanonenv, {"alpha": ["pkg-alpha"], "bravo": ["pkg-bravo"]})

        packages = list((tmp_path / ".packages").iterdir())
        assert len(packages) == 2, (
            f"Exactly 2 packages must exist in .packages/ after {_NUM_STABILITY_RUNS} installs, "

...
neither the CLI flag nor KANON_CATALOG_SOURCES is set, and that a fixture
...
class TestNoDefaultCatalog:
    ...
    def test_error_text_contains_env_var(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Error text must mention the KANON_CATALOG_SOURCES env var."""
        monkeypatch.delenv("KANON_CATALOG_SOURCES", raising=False)
        error_text = _fixture_command_handler("list")
        assert "KANON_CATALOG_SOURCES" in error_text
    ...
    def test_subprocess_raises_missing_catalog_source_error(self, tmp_path: pathlib.Path) -> None:
        """AC-CYCLE-001: subprocess with no KANON_CATALOG_SOURCES raises MissingCatalogSourceError."""
        script_path = tmp_path / "check_raises.py"
        script_path.write_text(
            "import os\n"
            "os.environ.pop('KANON_CATALOG_SOURCES', None)\n"

...
class TestRemoveDryRunBasic:
    ...
    def test_dry_run_exits_zero(self, tmp_path: pathlib.Path) -> None:
        """--dry-run exits 0."""
        kanon_file = tmp_path / ".kanon"
        kanon_file.write_text(_STANDARD_HEADER_LF + _FOO_BAR_BLOCK_LF)
    ...
    def test_dry_run_prints_minus_prefixed_lines(self, tmp_path: pathlib.Path) -> None:
        ...
        assert "-KANON_SOURCE_foo_bar_REF=refs/tags/1.0.0" in result.stdout
        assert "-KANON_SOURCE_foo_bar_PATH=repo-specs/foo-marketplace.xml" in result.stdout
        assert "-KANON_SOURCE_foo_bar_NAME=foo_bar" in result.stdout
        assert "-KANON_SOURCE_foo_bar_GITBASE=https://example.com" in result.stdout
    ...
    def test_dry_run_multi_source(self, tmp_path: pathlib.Path) -> None:
        ...
        content = (
            _STANDARD_HEADER_LF + _FOO_BAR_BLOCK_LF + "KANON_SOURCE_baz_qux_URL=https://example.com/baz.git\n"
...
class TestRemoveDryRunACCycle001:
    ...
    def test_dry_run_shows_five_minus_lines_and_file_unchanged(self, tmp_path: pathlib.Path) -> None:
        ...
        kanon_file = _build_crlf_fixture(tmp_path, extra_blanks_between_header_and_block=3)
        sha_before = _file_sha256(kanon_file)
